...
if __name__ == "__main__":
    ...
    Save_dir  = 'results/'
    Json_dirs = ['Synthetic_Json_Test_Conv', 'PMC_Json_Test']
    Img_dirs  = ['Synthetic_Img_Test', 'PMC_Img_Test']
    Re_Axis   = ['Horizontal box','Grouped horizontal bar','Stacked horizontal bar', 'horizontal bar']
    for ind, json_dir in enumerate(Json_dirs):
        Json_files = os.listdir(Data_dir + json_dir + '/')
        for json_file in Json_files:
            json_path = Data_dir + json_dir + '/' + json_file
            print(json_path)
            
            if(ind == 0):
                img_path  = Data_dir + Img_dirs[ind] + '/' + json_file.replace('json', 'png')
            else:
                img_path  = Data_dir + Img_dirs[ind] + '/' + json_file.replace('json', 'jpg')
                
            if(ind == 0 and (Img_dirs[ind] + '/' + json_file.replace('json', 'png') not in Val_Json.keys())):
                continue
            if(ind == 1 and (Img_dirs[ind] + '/' + json_file.replace('json', 'jpg') not in Val_Json.keys())):
                continue
            ...
            if(ind == 0):
                ...
                roles = json_data['task6']['input']['task3_output']['text_roles']
                ctype = json_data['task6']['input']['task1_output']['chart_type']
                boxes = json_data['task6']['input']['task2_output']['text_blocks']
                gt_xs = json_data['task6']['input']['task4_output']['axes']["x-axis"]
                gt_ys = json_data['task6']['input']['task4_output']['axes']["y-axis"]
            ...
            json.dump(json_data, open(Save_dir + json_dir + '/' + json_file, 'w'), indent=4)
