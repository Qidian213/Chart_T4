...
class DLASeg(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.last_level - self.first_level):
            y.append(x[i].clone())
        self.ida_up(y, 0, len(y))  ### 4s
        
        z = {}
        for head in self.heads:
            z[head] = self.__getattr__(head)(y[-1])
            if('heatmap' in head):
                z[head] = z[head].sigmoid()
                z[head] = torch.clamp(z[head], min=1e-4, max=1-1e-4)
        return z
...
# def get_pose_net(num_layers=34, heads={'heatmap':2, 'reg': 2}, down_ratio=4, head_conv=256):
