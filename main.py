...
class Mainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        for step,(images, heatmap_gt, heatmap_posweight, heatmap_offset, heatmap_off_mask, tag_offset ,num_objs) in enumerate(self.Data_dict['Train_loader']):
    ...
    def val_epoch(self, epoch):
        ...
        log_loss = 0
        with torch.no_grad():
            for step,(images, heatmap_gt, heatmap_posweight, heatmap_offset, heatmap_off_mask, tag_offset, num_objs) in enumerate(self.Data_dict['Val_loader']):
                batch_objs = sum(num_objs)
                if batch_objs == 0:
                    batch_objs = 1
