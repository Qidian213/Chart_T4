...
def detect(models, image, img_path, threshold=0.3):
    ...
    Tpoints[0].append([378,740])
    Tscores[0].append(1.0)
...
if __name__ == "__main__":
    ...
    model_1.eval()
    model_1.cuda()
    model_1.load_param("work_space/DLA_34_2020-10-29-13-53-55/Epoch_best.pth")

    cfg_2   = Cfg_Opts2()
    model_2 = Get_model(cfg_2)
    model_2.eval()
    model_2.cuda()
    model_2.load_param("work_space/DLA_34_2020-10-26-11-29-15/Epoch_best.pth")

    cfg_3   = Cfg_Opts3()
    model_3 = Get_model(cfg_3)
    model_3.eval()
    model_3.cuda()
    model_3.load_param("work_space/DLA_34_2020-10-24-11-53-34/Epoch_best.pth")

    sp_files = open("results/T4_lits.txt","r")
    sp_lines = sp_files.readlines()
    ...
    for line in sp_lines:
        line = line.strip()
        if('PMC' in line):
            PMC_Test_Json_Files.append(line[:-4] + '.json')
           # print(line, line[:-4] + '.json')
        else:
            SYN_Test_Json_Files.append(line[:-4] + '.json')
...
#### PMC
    ...
    PMC_Test_Out_Dir  = '/data/zzg/ICPR_Chart/AE_Chart/results/PMC_Test_SP/'
   ...
   # PMC_Test_Json_Files = os.listdir(PMC_Test_Json_Dir)
    for json_file in PMC_Test_Json_Files:
        ...
        Tscores, Tpoints = detect([model_1, model_2,model_3], Image, Img_path)
      ...
      #  axes['x-axis'].append({"id": 12, "tick_pt": {"x": int(224), "y": int(380)}})
        ...
        # for box in Need_match:
            ...
            # xp = (bb['x0'] + bb['x1'] + bb['x2'] + bb['x3'])/4.0
            # yp = (bb['y0'] + bb['y1'])/2.0
            # cv2.circle(Image, (int(xp), int(yp)), 2, (0, 255, 255), -1)
            # cv2.putText(Image, f'{id}', (int(xp+10), int(yp+15)), cv2.FONT_HERSHEY_COMPLEX, 0.5,(0, 0, 255), 1)
        ...
        for box in Need_match:
            ...
            for point in Tpoints[0]:
                # xp = (bb['x0'] + bb['x1'])/2.0
                # yp = (bb['y0'] + bb['y1'])/2.0
                dist = ((point[0] - xp)**2+(point[1] - yp)**2)**0.5
                dist = abs(point[0] - xp)
                if(dist < mdist):
                    mdist = dist
                    mcls  = 0
        ...
        cv2.imwrite('outs_sp/' + Img_path.split('/')[-1],Image)
...
#### SYN
    ...
    SYN_Test_Out_Dir  = '/data/zzg/ICPR_Chart/AE_Chart/results/SYN_Test_SP/'
 ...
 #       axes['x-axis'].append({"id": 12, "tick_pt": {"x": int(225), "y": int(380)}})
        ...
        for box in Need_match:
            ...
            yp = (bb['y0'] + bb['y1'] + bb['y2'] + bb['y3'])/4.0
            cv2.circle(Image, (int(xp), int(yp)), 2, (0, 255, 255), -1)
            cv2.putText(Image, f'{id}', (int(xp), int(yp)), cv2.FONT_HERSHEY_COMPLEX, 0.75,(0, 0, 255), 1)
