...
def augmentWithFlip(image, points):
    new_points = []
    ...
    for group in points:
        ...
        for point in group:
            x, y, bb = point
            x = width - x 
            if('height' in bb.keys()):
                x0 = bb['x0']
                bb['x0']    = width - bb['x0']
                bb['width'] = width - 2*bb['x0'] - bb['width']
                new_group.append([x,y,bb])
            else:
                bb['x0']    = width - bb['x0']
                bb['x1']    = width - bb['x1']
                bb['x2']    = width - bb['x2']
...
class LDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #if(randrf(0,1)<0.5):
            # image, info_points = augmentWithFlip(image, [x_points, y_points]) 
            # x_points, y_points = info_points
        
        # for point in x_points:
            # cv2.circle(image, (int(point[0]), int(point[1])), 5, (0, 0, 255), -1)
        # for point in y_points:
            # cv2.circle(image, (int(point[0]), int(point[1])), 5, (0, 0, 255), -1)
        # cv2.imwrite(f"imgs/" + File_path.split('/')[-1], image)
...
###
       ...
       # if(self.mode == 'train'):
        ...
        for point in tx_points:
            ...
            # gaussian_map = draw_truncate_gaussian(heatmap_gt[2, :, :], (cx, cy), self.radius, self.radius)
            # draw_gaussian(heatmap_posweight[2, :, :], (cx, cy), self.posweight_radius)
